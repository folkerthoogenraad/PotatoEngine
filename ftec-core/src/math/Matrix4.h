...
namespace ftec {
	...
	struct Matrix4
	{
		...
		Matrix4(const Matrix4<T> &other) : elements(other.elements) {}
		Matrix4(Matrix4<T> &&other) : elements(std::move(other.elements)) {}

		~Matrix4()
		{
			elements.~array<T, 4 * 4>();
		}
		...
		static Matrix4<T> translation(const Vector3<T>& t)
		{
			return translation(t.x, t.y, t.z);
		}
		static Matrix4<T> scaled(const Vector3<T>& s)
		{
